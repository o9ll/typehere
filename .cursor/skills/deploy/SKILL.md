...
```bash
rm -rf "/Applications/Type"
cp -R "release/0.0.1/mac-arm64/Type" "/Applications/Type"
```

3. Verify the app was installed:

```bash
ls -la "/Applications/Type"
```
...
- The output path is `release/0.0.1/mac-arm64/Type` on Apple Silicon. If the build output differs (e.g. `release/0.0.1/mac/Type` on Intel), adjust the copy source accordingly. Check `release/0.0.1/` contents if the expected path doesn't exist.
